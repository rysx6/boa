...
let url = 'https://appcms.acg.party';
...
//推荐
let videos = res.recommend_list.map(item => ({
    vod_id: item.vod_id,
...
async function detail (id) {
  ...
  let play_url = res.vod_play_list.map(play => {
    ...
    return play.urls.map(item => {
        let name = item.name;
        let url = item.url;
        let urls = p ? `${p}|${url}|${item.token}` : url;    // 如果 p 有值，则将其加到 url 前，并用 | 分隔，空则返回原 url
        return `${name}$${urls}`; 
    }).join('#');
}).join('$$$');
...
if (id.indexOf("|") > -1){
  ...
  let html = await request (`${url}/api.php/getappapi.index/vodParse?parse_api=${id.split("|")[0]}&url=${encodeURIComponent(AES2(id.split("|")[1]))}&token=${id.split("|")[2]}`);
